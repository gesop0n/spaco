...
import { useLandingPage } from "./_.hook";

export function LandingPage() {
  const { availabilityDialogRef, openAvailabilityDialog, closeAvailabilityDialog } =
    useLandingPage();
    ...
    <div className="flex min-h-svh flex-col">
      ...
      <header className="relative z-1 border-b border-border bg-surface">
        <div className="mx-auto flex min-h-22 `w-[calc(100%-96px)] max-w-280 items-center justify-between phone:min-h-18 phone:w-[calc(100%-40px)]">
          ...
          <button
            className="min-h-11 rounded-lg border-0 bg-transparent px-4 py-2.5 text-sm leading-normal font-[650] text-accent transition-[background] duration-160 ease-[ease] hover:bg-accent-soft motion-reduce:transition-none phone:px-2.5 phone:text-[13px]"
            type="button"
      ...
      <main
        id="main"
        className="relative isolate flex flex-1 items-center justify-center overflow-hidden bg-[radial-gradient(ellipse_at_50%_40%,#edf2ff_0%,var(--bg)_65%)] px-6 pt-16 pb-9.5 after:absolute after:-bottom-22 after:left-1/2 after:-z-1 after:h-35 after:w-[150%] after:-translate-x-1/2 after:rounded-[50%_50%_0_0] after:bg-surface after:content-[''] min-[1600px]:pt-22 min-[1600px]:pb-16 phone:px-5 phone:pt-13 phone:pb-5.5"
        tabIndex={-1}
      >
      ...
      <dialog
        ref={availabilityDialogRef}
        className="m-auto max-h-[calc(100svh-48px)] w-[min(420px,calc(100%-40px))] rounded-[18px] border border-border bg-surface px-7 pt-9 pb-7 text-center text-fg-strong shadow-dialog backdrop:bg-[#1e2b44]/28 backdrop:backdrop-blur-[5px]"
        aria-labelledby="availability-title"
        aria-describedby="availability-description"
      >
        <div
          className="mx-auto mb-5 grid size-13 place-items-center rounded-[14px] bg-accent-soft text-accent"
          aria-hidden="true"
        >
          <svg
            className="size-6.5 stroke-current stroke-[1.7]"
            viewBox="0 0 24 24"
            fill="none"
            strokeLinecap="round"
            strokeLinejoin="round"
          >
            <path d="M20 8a8 8 0 1 0 0 8M20 3v5h-5M12 8v4l3 2" />
          </svg>
        </div>
        <h2 id="availability-title" className="m-0 text-[21px] font-bold">
          ただいま準備中です。
        </h2>
        <p id="availability-description" className="mt-3.5 mb-7 text-[13px] leading-[1.9] text-fg">
          ログイン機能は、もうしばらくお待ちください。
        </p>
        <button
          className="min-h-11.5 w-full rounded-[9px] border-0 bg-accent-soft text-sm leading-normal font-[650] text-accent hover:bg-accent-line"
          type="button"
