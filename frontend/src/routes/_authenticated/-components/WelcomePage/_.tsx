import { BookOpen, CalendarCheck2, RotateCcw, Search } from "lucide-react";

const upcomingFeatures = [
  { icon: CalendarCheck2, title: "今日の復習", description: "今日取り組む問題を、ひと目で。" },
  { icon: Search, title: "問題を探す", description: "もう一度解きたい問題を見つけよう。" },
  { icon: BookOpen, title: "復習リスト", description: "解き直したい問題を、自分のリストに。" },
];
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center gap-8 px-5 py-12 md:px-10 md:py-16">
      <section className="relative isolate overflow-hidden rounded-3xl border border-border bg-card p-7 shadow-card sm:p-12">
        ...
        <p className="mt-2 max-w-lg text-sm leading-7 text-muted-foreground">
          あなたのペースで、少しずつ。AtCoderの復習を続けるための学習スペースを準備しています。
        </p>
      </section>
      ...
      <section aria-labelledby="upcoming-title">
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          {upcomingFeatures.map(({ icon: Icon, title, description }) => (
            <article key={title} className="rounded-2xl border border-border bg-card/80 p-5">
